...
impl epi::App for MyApp {
    fn update(&mut self, ctx: &egui::CtxRef, _frame: &mut epi::Frame) {
        let frame = egui::Frame {
            fill: egui::Color32::from_black_alpha(200), // Dark background
            corner_radius: 10.0,
            stroke: egui::Stroke::new(1.0, egui::Color32::from_rgb(200, 200, 200)), // Light stroke
            ..Default::default()
        };

        egui::CentralPanel::default().frame(frame).show(ctx, |ui| {
            ui.style_mut().spacing.item_spacing = egui::vec2(0.0, 15.0);
            ui.style_mut().visuals.widgets.noninteractive.bg_fill = egui::Color32::from_black_alpha(180); // Darker background for non-interactive
            ui.style_mut().visuals.widgets.inactive.bg_fill = egui::Color32::from_black_alpha(255); // Light background for inactive widgets
            ui.style_mut().visuals.widgets.hovered.bg_fill = egui::Color32::from_rgb(60, 60, 60); // Dark hover color
            ui.style_mut().visuals.widgets.active.bg_fill = egui::Color32::from_rgb(80, 80, 80); // Dark active color
            ...
            ui.vertical_centered(|ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui.add_sized(
                        [width, 40.0],
                        egui::Button::new(if self.is_compression { "Compress" } else { "Decompress" })
                    ).clicked() {
                ...
                ui.horizontal(|ui| {
                    ui.add_space((ui.available_width() - width) / 2.0);
                    let label = egui::Label::new(&self.status);
                    ui.add_sized([width, 20.0], label.text_color(egui::Color32::from_rgb(255, 255, 255))); // Using from_rgb for white
                });
                ...
                ui.with_layout(egui::Layout::bottom_up(egui::Align::Center), |ui| {
                    ui.add_space(10.0);
                    let footer_label = egui::Label::new("Created by Marmik Mewada from India <3");
                    ui.add(footer_label.text_color(egui::Color32::from_rgb(200, 200, 200)));
                });
            });
        });
    }
